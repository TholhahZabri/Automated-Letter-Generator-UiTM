...
if(isset($_GET['lett_id']) && isset($_GET['type']))
{
	...
	if($_GET['type'] == "appc") //check if letter type is appreciation letter
	{
		...
		try
		{
			...
			//sql query
			$stmt = $conn->prepare("SELECT * FROM letter_appreciation WHERE lett_id = '{$lett_id}' AND SENDER = '{$userid}' ");
	...
	else if($_GET['type'] == "app") //check if letter type is appointment letter
	{
		...
		try
		{
			...
			//sql query
			$stmt = $conn->prepare("SELECT * FROM letter_appointment WHERE lett_id = '{$lett_id}' AND SENDER = '{$userid}' ");
